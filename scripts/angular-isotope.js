...
// Config
angular.module('iso.config', []).value('iso.config', { debug: true });

// Modules
angular.module('iso.directives', ['iso.services']);
angular.module('iso.services', []);
angular.module('iso',['iso.config','iso.directives','iso.services']);
...
.controller("isoSortByDataController", [
  "iso.config", "$scope", "optionsStore", function(config, $scope, optionsStore) {
    ...
    $scope.optSortData = function(index, item) {
      ...
      selector = $item.attr("iso-sel");
      type = $item.attr("iso-type");
      sortKey = $scope.getHash(selector);
      fun = ($item.attr("iso-option-convert") ? eval_("[" + $item.attr("iso-option-convert") + "]")[0] : null);
...
.directive("isoSortbyData", function() {
    return {
      ...
      link: function(scope, element, attrs) {
        ...
        optKey = optionSet.attr("iso-key");
        optEvent = "iso-opts";
        options = {};
        methSet = optionSet.find("[iso-sel]");
        methSet.each(function(index) {
          var $this;
          $this = angular.element(this);
          return $this.attr("iso-sortby-key", scope.getHash($this.attr("iso-sel")));
        });
...
.directive("isoKind", ['optionsStore', 'iso.topics', function(optionsStore, topics) {
  return {
    restrict: "A",
    controller: "isoSortByDataController",
    link: function(scope, element, attrs) {
      ...
      createOptions = function(item) {
        ...
        if (item) {
          option = {};
          virtualSortByKey = item.attr("iso-sortby-key");
          ascAttr = item.attr("iso-option-ascending");
          key = virtualSortByKey || item.attr("iso-sel");
          if (virtualSortByKey) {
...
angular.module("iso.services", ["iso.config"], ['$provide', function($provide) {
